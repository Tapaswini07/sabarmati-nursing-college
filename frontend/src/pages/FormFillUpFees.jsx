...
const INSTITUTIONS = ["Sabarmati School of Nursing", "Sabarmati College of Nursing"];

function getInstitutionForCourse(course) {
  return ["ANM", "GNM"].includes(course)
...
const API_BASE = "/api/form-fill-up-fees";
...
function calculatePayableAmount(data = {}) {
  return Math.max(
    toAmount(data.formFillUpFee) + toAmount(data.examFee) + toAmount(data.otherFees) + toAmount(data.studentAdjustment),
    0
  );
}
...
async function printReceipt(fee) {
  ...
  const amount = toAmount(lastPayment.amount ?? fee.paidAmount ?? fee.totalPayable);
  const paymentDate = lastPayment.date || lastPayment.paymentDate || fee.paymentDate || fee.paidDate || new Date();
  const receiptNumber = lastPayment.receiptNumber || fee.receiptNumber || `SCN-FORM-${fee.studentId || "RECEIPT"}`;
  ...
  await printMoneyReceipt({
    student: {
      ...
      institution: fee.institution || "Sabarmati College of Nursing",
    },
    payment: {
      amount,
      ...
      paymentDate,
      transactionId,
      purpose: "Form Fill-Up Fee Payment",
      particular: `Form Fill-Up Fee - ${courseLabel || "Academic Fees"}`,
    },
    printWindow,
  });
}

export default function FormFillUpFees() {
  ...
  const fetchFees = useCallback(async () => {
    try {
      ...
      console.error("Error fetching form fill-up fees:", err);
      setError("Failed to load form fill-up fees. Please try again.");
      setFees([]);
    } finally {
      setLoading(false);
    }
  ...
  const validateFeePayload = (data) => {
    const formFillUpFee = Number(data.formFillUpFee);
    const examFee = Number(data.examFee || 0);
    const otherFees = Number(data.otherFees || 0);
    const studentAdjustment = Number(data.studentAdjustment || 0);
    const total = formFillUpFee + examFee + otherFees + studentAdjustment;

    if (!Number.isFinite(formFillUpFee) || formFillUpFee < 0) {
      return "Enter a valid form fill-up fee.";
    }
    if (!Number.isFinite(examFee) || examFee < 0) {
      return "Enter a valid exam fee.";
    }
  ...
  const handleAddFee = async (e) => {
    ...
    try {
      await apiRequest.post(API_BASE, {
        ...formData,
        formFillUpFee: Number(formData.formFillUpFee),
      ...
      console.error("Error adding form fill-up fee:", err);
      alert("Failed to add form fill-up fee. Please try again.");
    }
  };
  ...
  const handleEditFee = async (e) => {
    ...
    try {
      ...
      console.error("Error updating form fill-up fee:", err);
      alert("Failed to update form fill-up fee. Please try again.");
    }
  };

  const handleDeleteFee = async (id) => {
    if (!window.confirm("Are you sure you want to delete this form fill-up fee?")) return;
    try {
      await apiRequest.delete(API_BASE + "/" + id);
      fetchFees();
    } catch (err) {
      console.error("Error deleting form fill-up fee:", err);
      alert("Failed to delete form fill-up fee. Please try again.");
    }
  };
  ...
  const handleExportExcel = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", "form_fill_up_fees.xls");
  ...
  const openEditModal = (fee) => {
    ...
    setFormData({
      ...
      formFillUpFee: fee.formFillUpFee ?? fee.registrationFee ?? "",
      examFee: fee.examFee || "",
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          <RefreshCcw className="mx-auto h-12 w-12 animate-spin text-blue-600" />
          <p className="mt-4 text-slate-600">Loading form fill-up fees...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Form Fill-Up Fees</h1>
          <p className="text-slate-600">Manage student form fill-up fees, exam fees, and payments</p>
        </div>
      ...
      <div className="flex flex-wrap gap-4">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="h-12 w-full rounded-2xl border border-[#dce7f4] bg-[#f8fbff] pl-12 pr-4 text-sm text-slate-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.75)] transition focus:border-blue-300 focus:bg-white focus:ring-4 focus:ring-blue-100 outline-none"
          />
        </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-200">
            {filteredFees.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-6 py-12 text-center text-slate-500">No form fill-up fees found</td>
              </tr>
            ) : (
              filteredFees.map((fee) => (
                <tr key={fee._id} className="transition hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm font-semibold text-slate-900">
                    {currencyFormatter.format(fee.totalPayable || 0)}
                    {toAmount(fee.studentAdjustment) !== 0 && (
                      <span className="mt-1 block text-xs font-medium text-blue-600">Custom adjusted</span>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openViewModal(fee)} className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-blue-600" title="View Details">
                      ...
                      {fee.paymentStatus === "Paid" && (
                        <button onClick={() => printReceipt(fee)} className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-purple-600" title="Print Receipt">
                          <Printer className="h-4 w-4" />
                        </button>
                      )}
                      <button onClick={() => openEditModal(fee)} className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-blue-600" title="Edit">
                        <Edit className="h-4 w-4" />
                      </button>
                      <button onClick={() => { setSelectedFee(fee); setShowInstallmentModal(true); }} className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-blue-600" title="Add Installment">
                      ...
                      <button onClick={() => openHistoryModal(fee)} className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-blue-600" title="Payment History">
                        <History className="h-4 w-4" />
                      </button>
                      {fee.paymentStatus === "Unpaid" && (
                        <>
                          <button onClick={() => handleSendReminder(fee, "email")} className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-blue-600" title="Send Email Reminder">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-800">Add Student Form Fill-Up Fee</h2>
            ...
            <form onSubmit={handleAddFee} className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="md:col-span-2">
                  ...
                  <select required value={formData.institution} onChange={(e) => updateStudentFeeForm({ institution: e.target.value })}
                    className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">
                ...
                <div>
                  ...
                  <select required value={formData.course} onChange={(e) => updateStudentFeeForm({ course: e.target.value, semester: "", institution: getInstitutionForCourse(e.target.value) })}
                    className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-slate-700">Form Fill-Up Fee</label>
                  <div className="relative">
                    <IndianRupee className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <input type="number" required min="0" value={formData.formFillUpFee} onChange={(e) => setFormData({ ...formData, formFillUpFee: e.target.value })}
                      className="w-full rounded-xl border border-slate-300 pl-10 pr-4 py-2.5 text-sm transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" />
                  </div>
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-slate-700">Exam Fee</label>
                  <div className="relative">
                    <IndianRupee className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <input type="number" min="0" value={formData.examFee} onChange={(e) => setFormData({ ...formData, examFee: e.target.value })}
                      className="w-full rounded-xl border border-slate-300 pl-10 pr-4 py-2.5 text-sm transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-blue-700">Total Payable</p>
                <p className="mt-1 text-xl font-bold text-blue-950">{currencyFormatter.format(payablePreview)}</p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="flex-1 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700">Add Fee</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showEditModal && selectedFee && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-800">Edit Form Fill-Up Fee</h2>
            ...
            <form onSubmit={handleEditFee} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="flex-1 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700">Update Fee</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showViewModal && selectedFee && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-800">Form Fill-Up Fee Details</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><p className="text-xs font-medium text-slate-500">Student ID</p><p className="text-sm font-semibold text-slate-900">{selectedFee.studentId}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Student Name</p><p className="text-sm font-semibold text-slate-900">{selectedFee.studentName}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Institution</p><p className="text-sm font-semibold text-slate-900">{selectedFee.institution || "-"}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Course</p><p className="text-sm font-semibold text-slate-900">{selectedFee.course}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Semester</p><p className="text-sm font-semibold text-slate-900">{selectedFee.semester || "-"}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Form Fill-Up Fee</p><p className="text-sm font-semibold text-slate-900">{currencyFormatter.format(selectedFee.formFillUpFee || 0)}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Exam Fee</p><p className="text-sm font-semibold text-slate-900">{currencyFormatter.format(selectedFee.examFee || 0)}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Other Fees</p><p className="text-sm font-semibold text-slate-900">{currencyFormatter.format(selectedFee.otherFees || 0)}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Student Adjustment</p><p className="text-sm font-semibold text-slate-900">{currencyFormatter.format(selectedFee.studentAdjustment || 0)}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Total Payable</p><p className="text-sm font-semibold text-slate-900">{currencyFormatter.format(selectedFee.totalPayable || 0)}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Last Date</p><p className="text-sm font-semibold text-slate-900">{selectedFee.lastDate ? new Date(selectedFee.lastDate).toLocaleDateString() : "-"}</p></div>
                <div><p className="text-xs font-medium text-slate-500">Payment Status</p><StatusBadge status={selectedFee.paymentStatus} /></div>
                <div><p className="text-xs font-medium text-slate-500">Payment Method</p><p className="text-sm font-semibold text-slate-900">{selectedFee.paymentMethod || "-"}</p></div>
              </div>
              {selectedFee.paymentStatus === "Paid" && (
                <button onClick={() => printReceipt(selectedFee)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-purple-700">
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-4">
              <div className="rounded-xl bg-slate-50 p-4">
                ...
                <p className="text-sm font-semibold text-slate-900">{selectedFee.studentName} ({selectedFee.studentId})</p>
                <p className="mt-2 text-xs font-medium text-slate-500">Total Payable</p>
                <p className="text-sm font-semibold text-slate-900">{currencyFormatter.format(selectedFee.totalPayable || 0)}</p>
              </div>
                  ...
                  {selectedFee.paymentHistory.map((item) => (
                    <div key={item.receiptNumber || item.date + "-" + item.amount} className="rounded-xl border border-slate-200 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-sm font-semibold text-slate-900">{item.receiptNumber || "Receipt"}</p>
                        <p className="text-sm font-bold text-slate-900">{currencyFormatter.format(item.amount || 0)}</p>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mb-5 rounded-xl bg-slate-50 p-4 text-sm">
              <p className="font-semibold text-slate-900">{selectedFee.studentName}</p>
              <p className="mt-1 text-slate-600">{selectedFee.course} | {selectedFee.semester || "-"}</p>
              <div className="mt-3 grid grid-cols-2 gap-3">
                <div><p className="text-xs text-slate-500">Total Payable</p><p className="font-bold text-slate-900">{currencyFormatter.format(selectedFee.totalPayable || 0)}</p></div>
                <div><p className="text-xs text-slate-500">Pending</p><p className="font-bold text-rose-700">{currencyFormatter.format(Math.max((selectedFee.totalPayable || 0) - toAmount(selectedFee.paidAmount), 0))}</p></div>
              </div>
            </div>
            <form onSubmit={handlePaymentSubmit} className="space-y-4">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-slate-700">Amount</label>
                <input name="amount" type="number" min="1" max={Math.max((selectedFee.totalPayable || 0) - toAmount(selectedFee.paidAmount), 0)} defaultValue={Math.max((selectedFee.totalPayable || 0) - toAmount(selectedFee.paidAmount), 0)} required
              ...
              <div>
                ...
                <textarea name="notes" rows="3" className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" placeholder="Optional payment note" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={handleAddInstallment} className="space-y-4">
              <input name="amount" type="number" min="1" required placeholder="Installment amount" className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" />
              <input name="dueDate" type="date" required className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" />
              <button type="submit" className="w-full rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700">Save Installment</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={handleApplyLateFee} className="space-y-4">
              <input name="lateFeeAmount" type="number" min="1" required placeholder="Late fee amount" className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <button onClick={() => handleGenerateReport("form-fill-up")} className="rounded-xl border border-slate-200 p-4 text-left transition hover:bg-slate-50">
                <FileText className="mb-2 h-5 w-5 text-blue-600" />
                <p className="font-semibold text-slate-800">All Form Fill-Up Fees</p>
                <p className="text-xs text-slate-500">View all form fill-up fee records</p>
              </button>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {reportData.map((item, index) => (
                        <tr key={item._id || `${selectedReportType}-${index}`}>
                          ...
                          <td className="px-3 py-2">{currencyFormatter.format(item.totalPayable || item.totalAmount || 0)}</td>
